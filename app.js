...
const state = {
  ...
  liveTimer: null,
  bridge: { checked:false, available:false, platform:null },
  bridgeInfo: null, // last /wifi-info payload from the bridge, or null
...
function screenHome(){
  ...
  if (bi && bi.available){
    cards.push({ id:'ssid', label:'Network Name', value: bi.ssid || 'UNKNOWN', status: bi.ssid ? 'good' : 'unknown' });
    const sigVal = bi.signalDbm !== null && bi.signalDbm !== undefined ? bi.signalDbm + ' dBm' : (bi.signalPercent !== null && bi.signalPercent !== undefined ? bi.signalPercent + '%' : '—');
    const sigStatus = bi.signalDbm !== null ? (bi.signalDbm > -60 ? 'good' : (bi.signalDbm > -75 ? 'slow' : 'failed'))
      : (bi.signalPercent !== null ? (bi.signalPercent > 60 ? 'good' : (bi.signalPercent > 30 ? 'slow' : 'failed')) : 'unknown');
    cards.push({ id:'rssi', label:'Signal', value: sigVal, status: sigStatus });
  }
  cards.push(
...
function bridgeStatusStrip(){
  if (state.bridge.available){
    return `<div class="dim mono" style="font-size:11.5px;margin:10px 0 0;">🟢 Desktop Bridge connected (${state.bridge.platform}) — real SSID/signal shown below.</div>`;
  }
  return `<div class="dim mono" style="font-size:11.5px;margin:10px 0 0;">
    ⚪ Desktop Bridge not connected — SSID/signal not shown. <a href="#capabilities" style="color:var(--gold);">Set it up</a> to see real values.
  </div>`;
}
  ...
  document.querySelectorAll('[data-explain]').forEach(card => {
    card.addEventListener('click', () => {
      ...
      const map = {
        ...
        gateway: {},
        ssid: { available: !!(state.bridgeInfo && state.bridgeInfo.ssid), text: state.bridgeInfo ? state.bridgeInfo.ssid : null },
        rssi: { available: !!(state.bridgeInfo && (state.bridgeInfo.signalDbm !== null || state.bridgeInfo.signalPercent !== null)),
          text: state.bridgeInfo ? (state.bridgeInfo.signalDbm !== null && state.bridgeInfo.signalDbm !== undefined ? state.bridgeInfo.signalDbm + ' dBm' : (state.bridgeInfo.signalPercent + '%')) : null }
      };
...
/* ---------------- Desktop Bridge ---------------- */
async function checkBridgeAndUpdate(announce){
  const wasAvailable = state.bridge.available;
  const result = await PixelDiag.checkBridge();
  state.bridge = { checked:true, available: result.available, platform: result.platform || null };
  if (result.available){
    state.bridgeInfo = await PixelDiag.getBridgeWifiInfo();
    if (!wasAvailable && announce) { logEvent('Desktop Bridge connected — real SSID/signal now available.', 'good'); toast('Desktop Bridge connected.'); }
  } else {
    state.bridgeInfo = null;
    if (wasAvailable && announce) logEvent('Desktop Bridge disconnected.', 'warn');
  }
  if (['home','xray','capabilities'].includes((location.hash || '#home').slice(1).split('/')[0])) renderRoute();
}
...
function screenXray(){
  ...
  const bi = state.bridgeInfo;
  let apStatus = 'not_testable', apExplain = 'gateway';
  if (bi && bi.available){
    apExplain = 'rssi';
    if (bi.signalDbm !== null && bi.signalDbm !== undefined) apStatus = bi.signalDbm > -60 ? 'good' : (bi.signalDbm > -75 ? 'slow' : 'failed');
    else if (bi.signalPercent !== null && bi.signalPercent !== undefined) apStatus = bi.signalPercent > 60 ? 'good' : (bi.signalPercent > 30 ? 'slow' : 'failed');
    else apStatus = 'unknown';
  }
  const nodes = [
    ...
    { id:'ap', label:'ACCESS POINT', status: apStatus, explain: apExplain },
...
function bindXrayEvents(){
  document.querySelectorAll('.xray-node').forEach(n => n.addEventListener('click', () => {
    const id = n.dataset.node;
    const bi = state.bridgeInfo;
    let valueObj = {};
    if (id === 'rssi'){
      valueObj = { available: !!(bi && bi.available), text: bi ? (bi.signalDbm !== null && bi.signalDbm !== undefined ? bi.signalDbm + ' dBm' : (bi.signalPercent + '%')) : null };
    }
    explainModal(id, valueObj);
  }));
...
function liveValueFor(key){
  const v = state.liveValues;
  const bi = state.bridgeInfo;
  const rssiText = bi && bi.available ? (bi.signalDbm !== null && bi.signalDbm !== undefined ? `${bi.signalDbm} dBm` : (bi.signalPercent !== null && bi.signalPercent !== undefined ? `${bi.signalPercent}%` : null)) : null;
  const map = {
    ...
    rssi: rssiText,
    ssid: bi && bi.available && bi.ssid ? bi.ssid : null,
    ipFamily: v.ipFamily
  };
...
function screenCapabilities(){
  ...
  const bridgeRow = state.bridge.available
    ? `<div class="cap-row"><span class="cdot">🟢</span><span class="cname">Desktop Bridge</span><span class="cnote">connected — ${state.bridge.platform}</span></div>`
    : `<div class="cap-row"><span class="cdot">🔴</span><span class="cname">Desktop Bridge</span><span class="cnote">not connected</span></div>`;
  ...
  return `
    ...
    <div class="card">${bridgeRow}${rows}</div>
    <div class="btn-row"><button class="btn" id="btn-recheck-bridge">RE-CHECK BRIDGE</button></div>

    <h2 class="section-title">Desktop Bridge Setup</h2>
    <div class="explain-block">
      <p>Browsers never expose your real SSID or signal strength (RSSI) — that's a deliberate privacy boundary, not a bug. The optional Desktop Bridge is a small program you run yourself that reads that from your OS and hands it to this page over <span class="mono">127.0.0.1</span> only. Nothing leaves your machine.</p>
      <h4>Setup</h4>
      <p class="mono" style="font-size:13px;">1. <a href="./bridge-server.js" download>Download bridge-server.js</a><br>
      2. Requires Node.js installed on your computer.<br>
      3. Run: <b>node bridge-server.js</b><br>
      4. Come back here — Pixel detects it automatically within a couple of seconds.</p>
      <h4>Security</h4>
      <p>The bridge binds to 127.0.0.1 only (never reachable from your network), only answers two read-only endpoints, and only accepts requests from this app's domain or localhost — not from any random website.</p>
    </div>

    <h2 class="section-title">Architecture</h2>
    <p class="dim" style="font-size:13.5px;line-height:1.6;">PIXEL WiFi UI → Capability Detection → Available Platform APIs (+ optional Desktop Bridge) → Diagnostic Engine → Explanation Engine → Next Action.</p>
    ${companyFooter()}
  `;
}
function bindCapabilitiesEvents(){
  const btn = document.getElementById('btn-recheck-bridge');
  if (btn) btn.addEventListener('click', async () => { toast('Checking…'); await checkBridgeAndUpdate(true); });
}
function labelize(k){
...
function renderRoute(){
  ...
  else if (route === 'capabilities'){ main.innerHTML = screenCapabilities(); bindCapabilitiesEvents(); }
...
function init(){
  ...
  checkBridgeAndUpdate(true);
  setInterval(() => checkBridgeAndUpdate(true), 30000);
